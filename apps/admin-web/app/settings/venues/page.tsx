          ...
          {!showAdd ? (
            ...
            <div className="p-4 rounded-xl border border-border-light dark:border-border-dark space-y-3">
              ...
              <div>
                ...
                <PlaceAutocomplete value={addPlaceDesc} onSelect={handleAddPlace} placeholder="Search and select an address..." />
              </div>
              ...
              {venues.map((v) =>
                editingId === v.id ? (
                  <li key={v.id} className="p-4 rounded-xl border border-border-light dark:border-border-dark space-y-3">
                    ...
                    <div><p className="text-xs text-gray-500 mb-1">Current: {v.formatted_address || v.city || '—'}</p><PlaceAutocomplete value={editPlaceDesc} onSelect={handleEditPlace} placeholder="Search to change address (optional)" /></div>
                    <div className="flex gap-2">
                  ...
                  <li key={v.id} className="p-3 rounded-xl border border-border-light dark:border-border-dark flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-500 truncate">{[v.city, v.state].filter(Boolean).join(', ') || v.formatted_address || '—'}</div>
                    </div>
