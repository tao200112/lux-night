 ...
 * Weekly ticket config (7-day recurring)
 */
...
export default function WeekConfigPage() {
    ...
    const fetchConfig = async () => {
      try {
        const [weekRes, infoRes] = await Promise.all([
           fetch(`/api/admin/events/${eventId}/week?date=${selectedWeekDate}`),
  ...
  const handlePublish = async () => {
    ...
    try {
        const res = await fetch(`/api/admin/events/${eventId}/publish`, { method: 'POST' });
        const json = await res.json();
        if (json.error) throw new Error(json.error);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/events/${eventId}/week`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const syncMsg = json.stripe_sync?.status === 'failed'
          ? `\n\nWARNING: Stripe Sync Failed: ${json.stripe_sync.error}\nCheck your STRIPE_KEY settings.`
  ...
  if (loading) return (
    <PageContainer fullBleed className="bg-[#101022] text-white flex items-center justify-center">
      Loading...
    </PageContainer>
  );

  return (
    <PageContainer fullBleed className="bg-[#f6f6f8] dark:bg-[#101022] text-slate-900 dark:text-white font-sans pb-32">
      {/* Sticky Header */}
      <div className="sticky top-0 z-50 bg-[#111118]/95 backdrop-blur-md border-b border-[#343445] shadow-xl">
        <div className="flex items-center justify-between px-4 pt-3 pb-3">
          <Link href="/events" className="text-white flex items-center justify-center p-2 -ml-2 rounded-full hover:bg-white/10 transition">
