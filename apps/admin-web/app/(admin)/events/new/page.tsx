/**
 * Admin Create Event Page - Step 1
...
function NewEventForm() {
  ...
  const [merchantData, setMerchantData] = useState<{ name: string; timezone: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchMerchant = async () => {
      try {
        ...
        if (!json.success || !json.data) throw new Error('Failed to load merchant data');
        const m = json.data;
        const venue = m.venues?.[0];
        setMerchantData({ name: m.name, timezone: 'America/New_York' });
        if (venue) {
  ...
  const handleFileSelect = async (file: File) => {
    if (!file || file.size > 5 * 1024 * 1024) {
      if (file) alert('File size must be less than 5MB');
      return;
    }
    try {
      setUploading(true);
      ...
      const res = await fetch('/api/admin/uploads/poster', { method: 'POST', body: formData });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/events/${result.event.id}/week`);
    } catch (err: any) {
  ...
  if (!merchantId) {
    return (
      <PageContainer fullBleed className="bg-[#0A0A0A] text-white flex items-center justify-center p-4">
        <div className="text-center space-y-4">
          <p className="text-red-400">Merchant ID is missing.</p>
          <Link href="/merchants" className="text-primary hover:underline">Go to Merchants</Link>
        </div>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer fullBleed className="bg-[#0A0A0A] text-white font-sans flex flex-col relative overflow-x-hidden selection:bg-[#1313ec]/30 selection:text-white">
      <div className="sticky top-0 z-50 bg-[#0A0A0A]/80 backdrop-blur-md border-b border-[#262626]">
        <div className="flex items-center justify-between px-4 py-3 max-w-lg mx-auto w-full">
          <Link href={`/merchants/${merchantId}`} className="flex items-center justify-center text-[#9d9db9] active:text-white transition-colors">
      ...
      <main className="flex-1 flex flex-col px-4 pt-6 pb-32 space-y-8 max-w-lg mx-auto w-full">
        ...
        <section className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-[#111111] border border-[#262626] rounded-lg px-4 py-3.5 text-white placeholder-[#444444] focus:outline-none focus:border-[#1313ec] focus:ring-1 focus:ring-[#1313ec] transition-all text-base" placeholder="e.g. Summer Bass Night" type="text" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-[#9d9db9] uppercase tracking-wide ml-1">Subtitle</label>
            <input value={subtitle} onChange={(e) => setSubtitle(e.target.value)} className="w-full bg-[#111111] border border-[#262626] rounded-lg px-4 py-3.5 text-white placeholder-[#444444] focus:outline-none focus:border-[#1313ec] focus:ring-1 focus:ring-[#1313ec] transition-all text-base" placeholder="e.g. Special Guest DJ" type="text" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-[#9d9db9] uppercase tracking-wide ml-1">Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-[#111111] border border-[#262626] rounded-lg px-4 py-3 text-white placeholder-[#444444] focus:outline-none focus:border-[#1313ec] focus:ring-1 focus:ring-[#1313ec] transition-all text-base resize-none" placeholder="Describe the vibe, lineup, and dress code..." rows={4}></textarea>
          </div>
        </section>
        ...
        <section className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="space-y-1.5">
              ...
              <input value={venueName} onChange={(e) => setVenueName(e.target.value)} className="w-full bg-[#111111] border border-[#262626] rounded-lg px-4 py-3.5 text-white placeholder-[#444444] focus:outline-none focus:border-[#1313ec] focus:ring-1 focus:ring-[#1313ec] transition-all text-base" placeholder="e.g. Club Space" type="text" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#9d9db9] uppercase tracking-wide ml-1">Address</label>
              <input value={venueAddress} onChange={(e) => setVenueAddress(e.target.value)} className="w-full bg-[#111111] border border-[#262626] rounded-lg px-4 py-3.5 text-white placeholder-[#444444] focus:outline-none focus:border-[#1313ec] focus:ring-1 focus:ring-[#1313ec] transition-all text-base" placeholder="e.g. 34 NE 11th St" type="text" />
            </div>
          </div>
        </section>
      ...
      <footer className="fixed bottom-0 left-0 w-full z-40 bg-[#0A0A0A]/90 backdrop-blur-lg border-t border-[#262626] p-4 pb-6 safe-area-bottom">
        <div className="max-w-lg mx-auto w-full flex gap-3">
          <button onClick={handleSubmit} disabled={loading || !title || !posterUrl} className="flex-1 bg-[#1313ec] hover:bg-[#1313ec]/90 active:scale-[0.98] text-white font-bold text-base py-3.5 px-6 rounded-lg shadow-[0_0_15px_rgba(19,19,236,0.3)] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:shadow-none">
            {loading ? <span className="animate-spin h-5 w-5 border-2 border-white/30 border-t-white rounded-full"></span> : (<>Save Draft & Configure<span className="material-symbols-outlined text-[20px]">arrow_forward</span></>)}
          </button>
        </div>
      </footer>
    </PageContainer>
