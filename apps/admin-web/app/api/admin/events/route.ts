 ...
 * PATCH /api/admin/events - Batch update sort_order
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: merchantData, error: merchantError } = await supabase
      .from('merchants')
      .select(`id, region_id, venues:venues!venues_merchant_id_fkey(id)`)
      .eq('id', merchant_id)
      .single();
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let query = supabase
      .from('events_v2')
      .select(`*, merchants!inner (id, name, region_id)`)
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (errors.length > 0) {
      console.error('[PATCH events order] Supabase errors:', errors);
      return NextResponse.json(
