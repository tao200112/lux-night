...
-- 适用于在 Supabase Dashboard SQL Editor 中执行（无需登录）
--
-- 使用方法：
-- 1. 先查询并获取一个用户 ID（用于 created_by）：
--    SELECT id, email FROM auth.users ORDER BY created_at DESC LIMIT 1;
-- 2. 如果有用户，直接执行此脚本
-- 3. 如果没有用户，脚本会自动查找或提示创建
--

-- =========================================================
-- 步骤 1: 获取或创建用户 ID（用于 created_by）
-- =========================================================
...
BEGIN
  ...
  -- 方式 4: 如果还是找不到，提示用户手动指定
  IF v_user_id IS NULL THEN
    RAISE NOTICE '⚠️  警告：未找到任何用户。';
    RAISE NOTICE '';
    RAISE NOTICE '请先通过应用注册一个用户，或者手动指定 created_by 用户 ID。';
    RAISE NOTICE '';
    RAISE NOTICE '手动指定方式：';
    RAISE NOTICE '1. 查询用户：SELECT id, email FROM auth.users LIMIT 1;';
    RAISE NOTICE '2. 将下面的 v_user_id := NULL; 改为：v_user_id := ''YOUR_USER_ID''::UUID;';
    RAISE NOTICE '3. 重新执行脚本';
    RAISE NOTICE '';
    
    -- 暂时使用 NULL，后续会报错（但会给出明确提示）
    v_user_id := NULL;
  ELSE
  ...
  -- 检查是否有用户 ID
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION '无法自动获取用户 ID。请先通过应用注册一个用户，或者手动指定 created_by 用户 ID。参见脚本中的提示。';
  END IF;
