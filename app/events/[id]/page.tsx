          ...
          {ticketTypes.length === 0 ? (
            ...
            <div className="flex flex-col gap-4">
              {ticketTypes.map(tier => {
                ...
                const available = tier.inventory_limit === null ? Infinity : tier.inventory_limit - tier.sold_count;
                const isSoldOut = available === 0;
                const maxQuantity = Math.min(tier.redeem_limit, available, 10); // Limit to 10 per order

                return (
                  <div key={tier.id} className={`group relative overflow-hidden rounded-2xl bg-lux-card border border-white/10 transition-all duration-300 hover:border-primary/40 hover:bg-lux-card/80 p-5 ${tier.inventory_limit !== null && available < 10 ? '' : ''}`}>
                    {tier.inventory_limit !== null && available < 10 && !isSoldOut && (
                    ...
                    <div className="flex flex-col gap-5 relative z-10">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {tier.inventory_limit !== null && available < 10 && (
                        ...
                        <p className="text-sm text-lux-gray mt-1.5 leading-relaxed">
                          {tier.category === 'ENTRY' ? 'Entry to the main floor, live DJ set access, and open bar until midnight.' : 'Private booth for 4, 2 premium bottles, personal server, and skip-the-line.'}
                        </p>
                      </div>
