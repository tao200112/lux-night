...
export default function WorkspacesPage() {
  ...
  const handleSelectWorkspace = async () => {
    ...
    try {
      const res = await fetch('/api/internal/workspace/select', {
        method: 'POST',
  ...
  if (workspaces.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8">
        ...
        <button
          onClick={() => router.push('/internal/invite')}
          className="px-4 py-2 bg-primary text-white rounded-lg"
        >
          Enter Invite Code
        </button>
  ...
  return (
    <div className="relative w-full max-w-md mx-auto min-h-screen bg-background-dark text-white p-8">
