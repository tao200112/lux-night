...
## 问题
远程数据库中的 `create_staff_invite` 函数仍有 `v_venue` 未赋值错误。

## 解决方案：立即在 Supabase Dashboard 执行
...
BEGIN
  ...
  -- 9. 返回结果
  -- 关键修复：使用 CASE 语句，只在 p_venue_id IS NOT NULL 时访问 v_venue.name
