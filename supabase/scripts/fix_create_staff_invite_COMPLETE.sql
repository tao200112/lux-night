-- =========================================================
-- 完整修复 create_staff_invite 函数 - 立即执行此脚本
-- 问题：v_venue 未赋值时访问 v_venue.name
-- =========================================================

-- 首先删除旧函数（如果存在）
DROP FUNCTION IF EXISTS public.create_staff_invite(UUID, TEXT, INT, INT, UUID, TEXT);
...
DECLARE
  ...
  v_merchant RECORD;
  v_venue RECORD;
  v_venue_name TEXT;  -- 新增：单独变量存储 venue_name
BEGIN
  -- 1. 校验必须已登录
  v_user_id := auth.uid();
  ...
  -- 5. 校验 venue（如果指定）并获取 venue_name
  v_venue_name := NULL;  -- 初始化为 NULL
  ...
  -- 9. 返回结果
  -- 修复：使用单独的变量 v_venue_name，避免访问未赋值的 RECORD
  RETURN jsonb_build_object(
    ...
    'venue_id', p_venue_id,
    'venue_name', v_venue_name,  -- 使用单独变量
